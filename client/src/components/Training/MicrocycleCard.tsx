...
export function MicrocycleCard(props: MicrocycleCardProps) {
	const startDate = props.microcycleData?.cycle_start_date
		? dayjs.tz(props.microcycleData.cycle_start_date).format('MM/DD/YYYY')
		: '';
	const endDate = props.microcycleData?.cycle_end_date
		? dayjs.tz(props.microcycleData.cycle_end_date).format('MM/DD/YYYY')
		: '';
	const dateString = startDate !== endDate ? `${startDate} - ${endDate}` : startDate;

	const currentDate = dayjs();
	const isCycleCurrent = currentDate.isBetween(
		dayjs(props.microcycleData?.cycle_start_date),
		dayjs(props.microcycleData?.cycle_end_date),
		'day',
		'[]'
	);
	const cardStatus = isCycleCurrent
		? 'success'
		: currentDate.isBefore(dayjs(props.microcycleData?.cycle_start_date))
			? 'warning'
			: 'default';
	const statusText = isCycleCurrent
		? 'Current'
		: currentDate.isBefore(dayjs(props.microcycleData?.cycle_start_date))
			? 'Pending'
			: 'Completed';
	...
	return (
		<Card
			...
			size="small"
			title={
				props.microcycleData?.cycle_name && props.microcycleData?.cycle_name.trim() !== ''
					? props.microcycleData.cycle_name
					: `Phase ${props.index}`
			}
