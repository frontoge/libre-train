...
export function MesocycleCard(props: MesocycleCardProps) {
	...
	const isCycleCurrent = currentDate.isBetween(
		dayjs(mesocycleData?.cycle_start_date),
		dayjs(mesocycleData?.cycle_end_date),
		'day',
		'[]'
	);
	const cardStatus = isCycleCurrent
		? 'success'
		: currentDate.isBefore(dayjs(mesocycleData?.cycle_start_date))
			? 'warning'
			: 'default';
	...
	return (
		<Card
			...
			size="small"
			title={
				mesocycleData?.cycle_name && mesocycleData?.cycle_name.trim() !== ''
					? mesocycleData.cycle_name
					: `Phase ${props.index}`
			}
