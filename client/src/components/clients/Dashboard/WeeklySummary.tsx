				...
				<>
					...
					>
						<Card variant="outlined" className="weekly-summary-card">
							<Statistic
								title="Weight"
								value={summaryState?.weight}
								precision={1}
								suffix={`lbs ${formatStatisticDiff(summaryState?.weightDiff, 1)}`}
							/>
						</Card>
						<Card variant="outlined" className="weekly-summary-card">
							<Statistic
								title="Body Fat"
								value={summaryState?.bodyFat}
								precision={1}
								suffix={`% ${formatStatisticDiff(summaryState?.bodyFatDiff, 1)}`}
							/>
						</Card>
						<Card variant="outlined" className="weekly-summary-card">
							<Statistic
								title="Lean Mass"
								value={summaryState?.leanMass}
								precision={1}
								suffix={`lbs ${formatStatisticDiff(summaryState?.leanMassDiff, 1)}`}
							/>
						</Card>
						<Card variant="outlined" className="weekly-summary-card">
							<Statistic
								title="Calories"
								value={summaryState?.calories}
								precision={0}
								suffix={`kcal ${formatStatisticDiff(summaryState?.caloriesDiff)}`}
							/>
						</Card>
					</div>
					<div
					...
					>
						<Card variant="outlined" className="weekly-summary-card">
							<Statistic
								title="Calorie Deficiency"
								value={summaryState?.calorieDeficiency}
								precision={0}
								suffix="kcal"
							/>
						</Card>
