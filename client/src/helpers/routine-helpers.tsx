 ...
 */
export function handleRoutineDrop(
	routineTree: WorkoutRoutineTreeNode[],
	sourceNode: WorkoutRoutineTreeNode,
	targetNode: WorkoutRoutineTreeNode,
	toGap: boolean
): WorkoutRoutineTreeNode[] {
	...
	if (
		sourceNode.nodeType === WorkoutNodeType.Category
		|| (sourceNode.nodeType === WorkoutNodeType.Group && targetDepth === 3)
	) {
		return updatedTree;
	}
	...
	// Dragging a group node
		...
		if (targetNode.nodeType === WorkoutNodeType.Group) {
			...
			if (toGap) {
				...
				const newLength = updatedTree[targetPos[0]].children?.[targetPos[1]].children?.unshift({
					...sourceNode,
					restAfter: 0,
				});
				(updatedTree[targetPos[0]] as WorkoutRoutineCategoryNode).children[targetPos[1]].title =
					newLength && newLength > 2 ? 'Circuit' : 'Superset';
			}
		} else {
			// Into group
			if (targetDepth === 3) {
				// Insert after target node
				updatedTree[targetPos[0]].children?.[targetPos[1]].children?.splice(targetPos[2] + 1, 0, {
					...sourceNode,
					restAfter: 0,
				});
				...
				if (toGap) {
					...
					updatedTree[targetPos[0]].children?.splice(targetPos[1], 1, {
						...
						children: [
							{ ...targetNode, restAfter: 0 },
							{ ...sourceNode, restAfter: 0 },
						],
	...
	if (sourceDepth === 3) {
		...
		const sourceGroupNode = getNodeAtPosition([updatedSourceGroupPosition[0], updatedSourceGroupPosition[1]], updatedTree) as
			| WorkoutRoutineGroupNode
			| undefined;
		(updatedTree[updatedSourceGroupPosition[0]] as WorkoutRoutineCategoryNode).children[updatedSourceGroupPosition[1]].title =
			sourceGroupNode && sourceGroupNode.children.length > 2 ? 'Circuit' : 'Superset';
		if (sourceGroupNode?.children?.length === 1) {
			...
			updatedTree[updatedSourceGroupPosition[0]].children?.splice(
				updatedSourceGroupPosition[1],
				0,
				remainingExerciseNode as WorkoutRoutineTreeNode
			);
		}
	}
...
export const getExerciseFormValuesFromNode = (
	node: WorkoutRoutineExerciseNode
): RoutineExerciseCreateEditFormProps['initialValues'] => {
	return {
